...
export default function AnalyticsPage() {
  const { radius, accentColor } = useThemeStore();
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500 overflow-hidden">
      ...
      {/* Unified Metric Grid Card */}
      <div className="rounded-3xl bg-card border border-border overflow-hidden shadow-sm divide-y divide-border/30 dark:divide-zinc-800/40">
        <div className="grid grid-cols-2 divide-x divide-border/30 dark:divide-zinc-800/40">
          <div className="p-4.5 flex flex-col justify-center bg-card/30">
            <p className="text-[10px] uppercase text-muted-foreground font-bold mb-1 flex items-center gap-1.5"><TrendingUp className="w-3.5 h-3.5 text-primary"/> Income</p>
            <h3 className="font-heading text-lg font-bold text-foreground">₹{totalIncome.toLocaleString('en-IN')}</h3>
          </div>
          <div className="p-4.5 flex flex-col justify-center bg-card/30">
            <p className="text-[10px] uppercase text-muted-foreground font-bold mb-1 flex items-center gap-1.5"><TrendingDown className="w-3.5 h-3.5 text-destructive"/> Expense</p>
            <h3 className="font-heading text-lg font-bold text-foreground">₹{totalExpense.toLocaleString('en-IN')}</h3>
          </div>
        </div>
        <div className="grid grid-cols-2 divide-x divide-border/30 dark:divide-zinc-800/40">
          <div className="p-4.5 flex flex-col justify-center bg-card/30">
            <p className="text-[10px] uppercase text-muted-foreground font-bold mb-1 flex items-center gap-1.5"><Wallet className="w-3.5 h-3.5 text-emerald-400"/> Net Savings</p>
          ...
          <div className="p-4.5 flex flex-col justify-center relative overflow-hidden bg-card/30">
            <div className="absolute right-[-10px] bottom-[-10px] opacity-[0.02] text-foreground"><Target className="w-24 h-24"/></div>
            <p className="text-[10px] uppercase text-muted-foreground font-bold mb-1 flex items-center gap-1.5"><Target className="w-3.5 h-3.5 text-cyan-400"/> Savings Rate</p>
      ...
      {timelineData.length > 0 && (
         <section className="w-full py-2 px-1">
          <h3 className="font-heading text-xs font-bold text-muted-foreground uppercase tracking-widest mb-5 flex items-center gap-2 px-1"><Calendar className="w-4 h-4 text-primary"/> Cash Flow Trend</h3>
          ...
          <div className="flex flex-col gap-2 mt-2 max-h-80 overflow-y-auto pr-2 custom-scrollbar">
            {(() => {
              ...
              return pieData.map((entry, index) => {
                ...
                const percentage = totalExpenseValue > 0 ? Math.round((entry.value / totalExpenseValue) * 100) : 0;
                
                return (
                  <div key={index} className="flex flex-col">
                    <button 
                      onClick={() => setExpandedCategory(isExpanded ? null : entry.name)}
                      className="flex flex-col w-full hover:bg-accent/40 p-2.5 rounded-xl transition-all duration-200 group/item text-left gap-2"
                    >
                      <div className="flex items-center gap-3 w-full">
                        <div className="w-2.5 h-2.5 rounded-full shrink-0 shadow-sm" style={{ backgroundColor: entry.color }}></div>
                        <span className="text-xs font-semibold text-foreground flex-1 truncate">{entry.name}</span>
                        <div className="flex items-center gap-1.5 shrink-0">
                          <span className="text-xs font-bold font-mono text-foreground/95">₹{entry.value.toLocaleString('en-IN', {maximumFractionDigits: 0})}</span>
                          <span className="text-[10px] text-muted-foreground font-black">({percentage}%)</span>
                      ...
                      <div className="w-full h-1 bg-accent/60 rounded-full overflow-hidden">
                        <div className="h-full rounded-full transition-all duration-500" style={{ width: `${percentage}%`, backgroundColor: entry.color }} />
                      </div>
                    </button>
              ...
              {topLocations.map((loc, i) => (
                <div key={i}>
                  ...
                  <div className="w-full h-1.5 bg-accent rounded-full overflow-hidden">
                     <div className="h-full rounded-full" style={{ width: `${(loc.amount / topLocations[0].amount) * 100}%`, backgroundColor: accentColor }} />
                  </div>
                </div>
              ))}
            </div>
